...
public class Trainer {
	...
	public Trainer(List<List<Double>> input_var, List<List<Double>> output_var, NeuralNetwork network_var, double learningRate_var, double momentum_var)
	{
		inputDataRows = input_var;
		outputDataRows = output_var;
		network = network_var;
		learningRate = learningRate_var;
		momentum = momentum_var;
	}
	
	public void runBackPropagation(int numOfIterations)
	{
		if(network == null)
			return;
	...
	public double testNetwork()
	{
		int count = 0;
		double tempMSE = 0.0;
		List<Double> outputRow = null;
		for(int i=0; i<inputDataRows.size(); i++)
		{
			List<Double> inputRow = inputDataRows.get(i);
			outputRow = outputDataRows.get(i);
			Matrix inputMatrix = new Matrix(inputRow.size()+1, 1);
			inputMatrix.set(0,0, 1.0);
		...
		return 0.5 * (tempMSE/outputRow.size());
	}
	...
	public void runErrorsBackWard(List<Double> output)
	{
		...
		for(int i = 0;i<network.layersList.size()-1;i++)
		{
			...
			NeuralLayer nextLayer = network.layersList.get(i+1);
			thisLayer.deltaWeightMatrix = nextLayer.errorVector.times(thisLayer.activationVector.transpose()).times(learningRate);
